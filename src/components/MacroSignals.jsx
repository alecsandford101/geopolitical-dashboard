...
import { MACRO_PLAYBOOK } from '../config/macroPlaybook.js'
...
// Commodities & FX desk — the macro sibling of the equities Decision desk. Same
// news-flow ranking, but instruments are directional, so each card carries a
// LONG/SHORT call (glyph + label, chrome — never a saturated hue) and its asset
// class. Educational, not investment advice.
export default function MacroSignals({ events }) {
  const ideas = useMemo(() => rankIdeas(events, MACRO_PLAYBOOK), [events])

  return (
    <section className="signals macro-signals">
      <div className="section-head">
        <div className="sh-left">
          <span className="eyebrow">Macro desk</span>
          <h2>Commodities &amp; FX in the crosshairs</h2>
        </div>
      ...
      {ideas.length === 0 ? (
        ...
        <div className="signals-grid">
          {ideas.map((it) => {
            const short = it.dir === 'short'
            return (
              <article className="sig" key={it.ticker} style={{ '--accent': it.hex }}>
                <div className="sig-top">
                  <span className="sig-ticker">{it.ticker}</span>
                  <span className={'sig-dir' + (short ? ' short' : ' long')}>
                    <span aria-hidden="true">{short ? '▼' : '▲'}</span> {short ? 'SHORT' : 'LONG'}
                  </span>
                ...
                <div className="sig-foot">
                  ...
                  <span className="sig-drivers">{it.klass} · {it.category}</span>
                </div>
              </article>
            )
          })}
      ...
      <p className="signals-note">
        Illustrative macro exposures derived from the current news flow — educational, not investment advice.
      </p>
    </section>
  )
}
